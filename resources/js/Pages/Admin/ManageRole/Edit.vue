...
const form = useForm({
    ...
    permissions: Object.fromEntries(
        Object.entries(props.defaultPermissions).map(([arrayName]) => {
            const availablePermissionsArray = props.availablePermissions[arrayName] || [];
            return [
                arrayName,
                availablePermissionsArray.map(permission => permission.name) || [],
            ];
        })
    ),
});
...
const selectAllPermissions = (arrayName) => {
    const selected = !selectAll.value[arrayName];
    selectAll.value[arrayName] = selected;

    if (selected) {
        for (const permission of props.defaultPermissions[arrayName]) {
            form.permissions[arrayName].push(permission.name);
        }
    } else {
        form.permissions[arrayName] = [];
    }
};

const submit = () => {
    form.put(route('role.update', props.data.id), {
        onFinish: () => {
            form.reset();
        },
    });
...
const clear = () => {
    router.get(route('role.edit', props.data.id), { },
    {
      replace: true,
    });
    search.value = ''
                        ...
                        <div class="col-span-6 py-4" v-for="(permissions, arrayName) in defaultPermissions" :key="arrayName">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-48 max-w-xl p-4">
                                <div v-for="permission in permissions" :key="permission">
                                    <div class="flex items-center">
                                        <Checkbox v-model:checked="form.permissions[arrayName]" :value="permission.name" class="w-5 h-5"/>
